...
-- SELECT count(number_of_reviews) FROM final_airbnb
...
-- SELECT neighbourhood, count(*) FROM final_airbnb
-- ORDER BY COUNT(*) DESC
-- LIMIT 1;

-- Investigate conclusion: I think it depends on what you are looking for in an airbnb if Williamsburg should be crowned most popular.  Harlem also has 16 listings, 
-- so just because a neighbourhood has many listings does not necessarily mean it is the most popular.  I would say that the availability would need to be considered 
-- to see how many people are booking in that neighbourhood and what the reviews say about each listing.  For instance if there was a 'star' or numerical rating system 
...
-- SELECT host_name, count(*) AS Most_Host_Listings FROM final_airbnb
-- GROUP BY host_name
